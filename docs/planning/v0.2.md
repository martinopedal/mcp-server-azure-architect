...
- **#96:** `feat(alz): expand vendored graph snapshot to all 132 queryable items` (Atlas, M effort, **LANDED**)
  - **MCP-shape rationale:** Expands data behind existing `alz_query_by_id` and `alz_query_list` tools. Checklist repo marked as merged catalogue with 173 queryable items. Temporarily disabled graph repo due to GUID collision per Decision 10/11.
  - **Depends on:** #125 (manifest v2 + refresh fix).
  - **Acceptance:** ✅ Vendored snapshot expanded to 173 queryable items (57% of 305 total). Manifest v2 with full metadata. SHA-256 hashes regenerated. Tests updated. CHANGELOG entry under [Unreleased].

- **#99:** `feat(alz): identity / RBAC drift queries (custom roles, classic admins, orphan MIs)` (Atlas, M effort, **LANDED**)
  ...
  - **Acceptance:** ✅ 5 IAM drift queries authored under `data/alz-queries/custom/` per ADR-006 (`source: "custom"`, empty-string `source_commit` sentinel): custom RBAC roles enumeration, classic administrators, orphan managed identities, privileged role assignments (Owner/Contributor/UAA), service principals with RBAC. Manifest v2 second `sources[]` entry with top-level `commit_sha: ""` so loader inherits the empty sentinel without per-query override. 5 new tests in `tests/test_custom_iam_queries.py`. CHANGELOG entry under [Unreleased]. Shipped via PR #128.
...
3. **Native IAM query provenance format:** ~~#99 ships native queries under `data/alz-queries/native/`. What citation header format?~~ **Resolved by ADR-006 + PR #128.** Custom queries live under `data/alz-queries/custom/` (not `native/`) with `source: "custom"`, `source_commit: ""` (empty-string sentinel), and a single-line `// Custom query authored in martinopedal/mcp-server-azure-architect` header followed by `// Issue #N` and `// Query ID: <guid>` lines.
