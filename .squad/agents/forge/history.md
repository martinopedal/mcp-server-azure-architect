...
**Measurements**:
- Before: 2,035,173 ┬Ás (2.04s)
- After: 1,878,198 ┬Ás (1.88s)
- Savings: 156,975 ┬Ás (157ms, 7.7% faster)
...
## 2026-01-13: pricing_estimate_workload Tool (Issue #44, PR #87)

**Context:** Implemented structured workload cost estimation tool composing pricing_lookup_sku results into multi-line estimates.

**WorkloadSpec shape rationale:**
- Used Pydantic v2 BaseModel for validation (already in deps via FastMCP)
- VmGroup: sku (str), count (int, ge=1), hours_per_month (float, ge=0, le=744, default=730)
- StorageItem: sku (str), capacity_gb (float, ge=0), region_override (str | None, default=None)
- WorkloadSpec: region (str), vms (list[VmGroup], default=[]), storage (list[StorageItem], default=[]), currency (str, default='USD')
- LineItem: sku, region, quantity, unit_price (Decimal), unit_of_measure, monthly_cost (Decimal), source_meter_id (str | None)
- CostEstimate: total_monthly (Decimal), currency, line_items, assumptions (list[str]), warnings (list[str])

**Decimal vs float choice:**
- Used Decimal for unit_price, monthly_cost, total_monthly to avoid floating-point precision issues in financial calculations
- Used float for quantity and capacity_gb (no precision concern for sizing metrics)
- Converted to Decimal via Decimal(str(value)) to avoid binary float repr issues

**Mocking patterns:**
- Reused existing _install_mock_transport pattern from test_pricing.py
- Each test defines a handler function that inspects request.url.params.get('$filter') to route responses
- Multi-SKU tests use substring matching (if 'D2s_v5' in filter_param:) to return different items per SKU
- Empty Items list ([]) simulates SKU not found, not an HTTP error

**Reusable patterns:**
- Variable naming to avoid mypy no-redef: use vm_cheapest and storage_cheapest instead of reusing cheapest in nested loops
- Pydantic model .model_dump(mode='python') returns dict with Decimal preserved (JSON would stringify Decimal)
- Tool registration: pass dict[str, Any] as parameter type, validate with WorkloadSpec.model_validate(spec) inside function body to preserve JSON Schema compatibility

**Validation gate passes:**
- ruff check and format: clean
- mypy on src/mcp_server_azure_architect/pricing.py and tests/test_pricing_estimate_workload.py: clean
- pytest: 8/8 tests pass (happy path, SKU not found, multi-region, non-hourly meter, empty spec, no unit_price, server tool, tool registration)
