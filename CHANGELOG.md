...
- **First public release.** Bumped `pyproject.toml` and runtime `__version__` to 0.2.0 (was 0.1.0 / 0.0.1 respectively). Pre-1.0 minor bump per ADR-005 covers all breaking changes accumulated in this release (manifest v2, source-field rename, custom-source enum).
- **MCP Registry publication wired into release pipeline.** New `publish-mcp-registry` job in `.github/workflows/release.yml` runs after `publish-pypi` succeeds, downloads `mcp-publisher` CLI, authenticates via GitHub OIDC (no secret needed), and publishes the server to the official MCP Registry under namespace `io.github.martinopedal/mcp-server-azure-architect`. Registry publication is now part of every tagged release. (Closes #93)
- **`server.json` at repo root** containing the MCP Registry manifest (schema 2025-12-11). Lists the PyPI package `mcp-server-azure-architect`, stdio transport, optional Azure DefaultAzureCredential environment variables (all `isRequired: false`), and `runtimeHint: uvx`. The registry namespace is bound to the `martinopedal` GitHub identity.
- **PyPI ownership verification marker** appended to `README.md` as an HTML comment (`<!-- mcp-name: io.github.martinopedal/mcp-server-azure-architect -->`). The MCP Registry fetches this from PyPI to verify package ownership. Invisible on GitHub render.
...
- **Release pipeline awk extraction hardened.** The `create-release` job in `.github/workflows/release.yml` previously used an awk range expression `/^## \[$VERSION\]/,/^## \[/` that collapsed the range to a single line (the version header itself) because the start and end patterns matched on the same line, then suppressed it, producing an empty GitHub Release body. Replaced with an explicit state machine that uses `index($0, "## [" version "] - ")` for unambiguous start matching, plus an `[ -s release-notes.md ] || exit 1` guard. Also dropped the wrong `v` prefix from the version variable (CHANGELOG follows Keep a Changelog `## [X.Y.Z]` convention).
